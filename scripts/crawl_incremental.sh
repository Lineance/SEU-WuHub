#!/bin/bash
#
# Crawl Incremental Script - Incrementally crawl and ingest all configured websites
#
# This script crawls all websites configured in config/websites/
# with max-pages=1 for incremental updates, then ingests into LanceDB.
#
# Usage: bash scripts/crawl_incremental.sh
...
LOG_FILE="${LOG_DIR}/crawl_incremental.log"
...
for config_file in ${CONFIG_FILES}; do
    ...
    CRAWL_LOG="${LOG_DIR}/crawl_${WEBSITE_NAME}.log"
    ...
    # Run crawler with max-pages=1 for incremental update
    cd "${PROJECT_ROOT}"
